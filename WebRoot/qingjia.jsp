...
<div class="header">
    ...
    <h1 class="page-title">请假条</h1>
</div>

        <ul class="breadcrumb">
    <li>论坛 <span class="divider">/</span></li>
    <li class="active">请假条</li>
</ul>
                    
<div class="well">
	<%-- <a href="uploadResource.jsp">上传资源</a>
	...
	<table class="table">
        ...
        <c:forEach var="al" items="${allist}">
            <tr>
                ...
                <td>${al.getAlsname()}</td>
                <td>${al.getAlclass()}</td>
                <td>${al.getAlreason()}</td>
                <td>${al.getAlstartdate()}-${al.getAlenddate()}</td>
                <td>${resource.getRclass()}</td>
            </tr>
        </c:forEach>
      </tbody>
	</table>	
